...
from .execution.inventory_vol_rebalance import InventoryVolRebalanceConfig, InventoryVolRebalanceManager
...
def initialize_inventory_vol_tables(results_db: str | Path) -> None:
    path = Path(results_db)
    path.parent.mkdir(parents=True, exist_ok=True)
    with sqlite3.connect(path) as conn:
        conn.executescript(
            """
            CREATE TABLE IF NOT EXISTS inventory_vol_positions (
                ...
                reductions_json TEXT NOT NULL,
                equity_curve_json TEXT NOT NULL,
                exposure_over_time_json TEXT NOT NULL,
                updated_at TEXT NOT NULL
            );

            CREATE TABLE IF NOT EXISTS inventory_vol_events (
            ...
            CREATE TABLE IF NOT EXISTS inventory_vol_research_metrics (
...
class InventoryVolRebalancePaperTrader:
    ...
    def run_once(self, *, limit: int = 250) -> dict[str, Any]:
        ...
        with PaperSignalStore(snapshot_db=self.snapshot_db, results_db=self.results_db) as store:
            ...
            for row in rows:
                ...
                decision = self.manager.on_book(
                    ...
                    atr_expansion=_raw_float(state, "atr_expansion"),
        ...
        summary.update({"snapshots_processed": processed, "signals_recorded": processed, "fills": fills, "reductions": reductions})
        return summary
    ...
    def _persist(self, conn: sqlite3.Connection) -> None:
        for position in self.manager.positions.values():
            ...
            conn.execute(
                """
                INSERT OR REPLACE INTO inventory_vol_positions (
                    event_key, market_ticker, market_close_time, strike,
                    held_above_qty, held_above_avg, held_below_qty, held_below_avg,
                    blended_basis, total_cost, mark_to_market_equity, realized_pnl,
                    unrealized_pnl, inventory_imbalance_ratio, inventory_imbalance_qty,
                    max_drawdown, time_to_expiry, distance_from_strike,
                    fills_json, reductions_json, equity_curve_json, exposure_over_time_json, updated_at
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    row["blended_basis"], row["total_cost"], row["mark_to_market_equity"], row["realized_pnl"],
                    row["unrealized_pnl"], row["inventory_imbalance_ratio"], row["inventory_imbalance_qty"],
                    row["max_drawdown"], row["time_to_expiry"], row["distance_from_strike"],
                    json.dumps(row["fills_json"], sort_keys=True), json.dumps(row["reductions_json"], sort_keys=True),
                    json.dumps(row["equity_curve_json"], sort_keys=True), json.dumps(row["exposure_over_time_json"], sort_keys=True),
        ...
        for event in self.manager.events:
            conn.execute(
                """
                INSERT INTO inventory_vol_events (
        ...
        updated_at = ""
        if metrics.get("equity_curve_over_intraperiod_oscillations"):
            updated_at = metrics["equity_curve_over_intraperiod_oscillations"][-1]["ts"]
        for name, value in metrics.items():
            conn.execute(
                """
                INSERT OR REPLACE INTO inventory_vol_research_metrics (metric_name, metric_json, updated_at)
                VALUES (?, ?, ?)
                """,
    ...
    def summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "research_metrics": self.manager.research_metrics(),
        }
